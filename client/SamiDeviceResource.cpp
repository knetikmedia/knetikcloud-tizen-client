...
JsonString* pUsersKey = new JsonString(L"users");
        ...
        if(pUsersVal != null) {
            ...
            jsonToValue(pUsers, pUsersVal, L"IList", L"SamiSimpleUserResource");
        }
        delete pUsersKey;
    }
}
...
SamiDeviceResource::asJsonObject() {
    ...
    JsonString *pUsersKey = new JsonString(L"users");
    pJsonObject->Add(pUsersKey, toJson(getPUsers(), "SamiSimpleUserResource", "array"));
...
IList*
SamiDeviceResource::getPUsers() {
    return pUsers;
}
void
SamiDeviceResource::setPUsers(IList* pUsers) {
    this->pUsers = pUsers;
}
