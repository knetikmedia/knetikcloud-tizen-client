...
void
getBatchProcessor(HttpResponse* pHttpResponse, void (* handler)(void*, SamiError*)) {
...
IList* 
SamiUtilBatchApi::getBatchWithCompletion(String* token, void (* success)(IList*, SamiError*)) {
  client = new SamiApiClient();

  client->success(&getBatchProcessor, (void(*)(void*, SamiError*))success);
  ...
  String url(L"/batch/{token}");

  String s_token(L"{");
  s_token.Append(L"token");
  s_token.Append(L"}");
  url.Replace(s_token, stringify(token, L"String*"));

  client->execute(SamiUtilBatchApi::getBasePath(), url, "GET", (IMap*)queryParams, mBody, (IMap*)headerParams, null, L"application/json");
  return null;
}
