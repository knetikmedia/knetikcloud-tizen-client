...
searchUsingPOSTProcessor(HttpResponse* pHttpResponse, void (* handler)(void*, SamiError*)) {
  ...
  if(code >= 200 && code < 300) {
    ...
    SamiPage«Map«string,object»»* out = new SamiPage«Map«string,object»»();
    jsonToValue(out, pJson, L"SamiPage«Map«string,object»»*", L"SamiPage«Map«string,object»»");
...
SamiSearchApi::searchUsingPOSTWithCompletion(String* type, SamiObject* query, Integer* size, Integer* page, String* order, void (* success)(SamiPage«Map«string,object»»*, SamiError*)) {
  client = new SamiApiClient();
