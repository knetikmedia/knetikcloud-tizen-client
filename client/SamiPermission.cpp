...
if(pPermission_role != null) {
        pPermission_role->RemoveAll(true);
        delete pPermission_role;
        pPermission_role = null;
    }
...
JsonString* pPermission_roleKey = new JsonString(L"permission_role");
        IJsonValue* pPermission_roleVal = null;
        pJsonObject->GetValue(pPermission_roleKey, pPermission_roleVal);
        if(pPermission_roleVal != null) {
            pPermission_role = new ArrayList();
            
            jsonToValue(pPermission_role, pPermission_roleVal, L"IList", L"SamiRole");
        }
        delete pPermission_roleKey;
...
SamiPermission::asJsonObject() {
    ...
    JsonString *pPermission_roleKey = new JsonString(L"permission_role");
    pJsonObject->Add(pPermission_roleKey, toJson(getPPermissionRole(), "SamiRole", "array"));
...
IList*
SamiPermission::getPPermissionRole() {
    return pPermission_role;
}
void
SamiPermission::setPPermissionRole(IList* pPermission_role) {
    this->pPermission_role = pPermission_role;
}
