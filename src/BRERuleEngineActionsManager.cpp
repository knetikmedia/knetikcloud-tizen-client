...
#include "BRERuleEngineActionsManager.h"
#include "NetClient.h"
#include "Helpers.h"
#include "Error.h"
...
BRERuleEngineActionsManager::BRERuleEngineActionsManager()
{
...
static gboolean __BRERuleEngineActionsManagerresponseHandler(gpointer data)
{
...
static gpointer __BRERuleEngineActionsManagerthreadFunc(gpointer data)
{
	RequestInfo *request = static_cast<RequestInfo*>(data);
	...
	g_idle_add(__BRERuleEngineActionsManagerresponseHandler, static_cast<gpointer>(request));
	...
	, void* userData, bool isAsync)
{
	...
	if(!isAsync){
		NetClient::easycurl(BRERuleEngineActionsManager::getBasePath(), url, myhttpmethod, queryParams,
		...
		requestInfo = new(nothrow) RequestInfo (BRERuleEngineActionsManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg, userData, reinterpret_cast<void(*)()>(handler), getBREActionsProcessor);;
		if(requestInfo == NULL)
			return false;

		thread = g_thread_new(NULL, __BRERuleEngineActionsManagerthreadFunc, static_cast<gpointer>(requestInfo));
		return true;
	}
}
...
bool BRERuleEngineActionsManager::getBREActionsAsync(char * accessToken,
	std::string filterCategory, std::string filterName, std::string filterTags, std::string filterSearch, 
	void(* handler)(std::list<ActionResource>, Error, void* )
	, void* userData)
{
...
bool BRERuleEngineActionsManager::getBREActionsSync(char * accessToken,
	std::string filterCategory, std::string filterName, std::string filterTags, std::string filterSearch, 
	void(* handler)(std::list<ActionResource>, Error, void* )
	, void* userData)
{
