...
void
PageResource«UsageInfo»::__cleanup()
{
	//if(content != NULL) {
...
void
PageResource«UsageInfo»::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<UsageInfo> new_list;
			UsageInfo inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("UsageInfo")) {
					jsonToValue(&inst, temp_json, "UsageInfo", "");
				} else {
...
PageResource«UsageInfo»::PageResource«UsageInfo»(char* json)
{
	this->fromJson(json);
}

char*
PageResource«UsageInfo»::toJson()
{
	JsonObject *pJsonObject = json_object_new();
	JsonNode *node;
	if (isprimitive("UsageInfo")) {
		list<UsageInfo> new_list = static_cast<list <UsageInfo> > (getContent());
		node = converttoJson(&new_list, "UsageInfo", "array");
	} else {
		...
		for (list<UsageInfo>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			UsageInfo obj = *it;
...
std::list<UsageInfo>
PageResource«UsageInfo»::getContent()
{
	return content;
}

void
PageResource«UsageInfo»::setContent(std::list <UsageInfo> content)
{
	this->content = content;
}

bool
PageResource«UsageInfo»::getFirst()
{
	return first;
}

void
PageResource«UsageInfo»::setFirst(bool  first)
{
	this->first = first;
}

bool
PageResource«UsageInfo»::getLast()
{
	return last;
}

void
PageResource«UsageInfo»::setLast(bool  last)
{
	this->last = last;
}

int
PageResource«UsageInfo»::getNumber()
{
	return number;
}

void
PageResource«UsageInfo»::setNumber(int  number)
{
	this->number = number;
}

int
PageResource«UsageInfo»::getNumberOfElements()
{
	return number_of_elements;
}

void
PageResource«UsageInfo»::setNumberOfElements(int  number_of_elements)
{
	this->number_of_elements = number_of_elements;
}

int
PageResource«UsageInfo»::getSize()
{
	return size;
}

void
PageResource«UsageInfo»::setSize(int  size)
{
	this->size = size;
}

std::list<Order>
PageResource«UsageInfo»::getSort()
{
	return sort;
}

void
PageResource«UsageInfo»::setSort(std::list <Order> sort)
{
	this->sort = sort;
}

long long
PageResource«UsageInfo»::getTotalElements()
{
	return total_elements;
}

void
PageResource«UsageInfo»::setTotalElements(long long  total_elements)
{
	this->total_elements = total_elements;
}

int
PageResource«UsageInfo»::getTotalPages()
{
	return total_pages;
}

void
PageResource«UsageInfo»::setTotalPages(int  total_pages)
{
