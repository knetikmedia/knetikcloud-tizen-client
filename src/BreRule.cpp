...
BreRule::__init()
{
	...
	//
	//
	//condition = new PredicateResource();
	//
	//
...
BreRule::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<ActionContext«object»> new_list;
			ActionContext«object» inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("ActionContext«object»")) {
					jsonToValue(&inst, temp_json, "ActionContext«object»", "");
				} else {
	...
	if (node !=NULL) {
		...
		if (isprimitive("PredicateResource")) {
			jsonToValue(&condition, node, "PredicateResource", "PredicateResource");
		} else {
			
			PredicateResource* obj = static_cast<PredicateResource*> (&condition);
...
BreRule::toJson()
{
	...
	JsonNode *node;
	if (isprimitive("ActionContext«object»")) {
		list<ActionContext«object»> new_list = static_cast<list <ActionContext«object»> > (getActions());
		node = converttoJson(&new_list, "ActionContext«object»", "array");
	} else {
		...
		for (list<ActionContext«object»>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			ActionContext«object» obj = *it;
	...
	if (isprimitive("PredicateResource")) {
		PredicateResource obj = getCondition();
		node = converttoJson(&obj, "PredicateResource", "");
	}
	else {
		
		PredicateResource obj = static_cast<PredicateResource> (getCondition());
		GError *mygerror;
		mygerror = NULL;
...
void
BreRule::setActions(std::list <ActionContext«object»> actions)
{
...
void
BreRule::setCondition(PredicateResource  condition)
{
