...
RevenueReportResource::__init()
{
	...
	//
	//
	//sales_average = double(0);
	//
	//
	//sales_total = double(0);
	//
}
...
RevenueReportResource::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("double")) {
			jsonToValue(&sales_average, node, "double", "");
		} else {
	...
	if (node !=NULL) {
		...
		if (isprimitive("double")) {
			jsonToValue(&sales_total, node, "double", "");
		} else {
...
void
RevenueReportResource::setSalesAverage(double  sales_average)
{
...
void
RevenueReportResource::setSalesTotal(double  sales_total)
{
