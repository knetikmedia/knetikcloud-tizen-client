...
void
PageResource«BreRule»::__cleanup()
{
	//if(content != NULL) {
...
void
PageResource«BreRule»::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<BreRule> new_list;
			BreRule inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("BreRule")) {
					jsonToValue(&inst, temp_json, "BreRule", "");
				} else {
...
PageResource«BreRule»::PageResource«BreRule»(char* json)
{
	this->fromJson(json);
}

char*
PageResource«BreRule»::toJson()
{
	JsonObject *pJsonObject = json_object_new();
	JsonNode *node;
	if (isprimitive("BreRule")) {
		list<BreRule> new_list = static_cast<list <BreRule> > (getContent());
		node = converttoJson(&new_list, "BreRule", "array");
	} else {
		...
		for (list<BreRule>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			BreRule obj = *it;
...
std::list<BreRule>
PageResource«BreRule»::getContent()
{
	return content;
}

void
PageResource«BreRule»::setContent(std::list <BreRule> content)
{
	this->content = content;
}

bool
PageResource«BreRule»::getFirst()
{
	return first;
}

void
PageResource«BreRule»::setFirst(bool  first)
{
	this->first = first;
}

bool
PageResource«BreRule»::getLast()
{
	return last;
}

void
PageResource«BreRule»::setLast(bool  last)
{
	this->last = last;
}

int
PageResource«BreRule»::getNumber()
{
	return number;
}

void
PageResource«BreRule»::setNumber(int  number)
{
	this->number = number;
}

int
PageResource«BreRule»::getNumberOfElements()
{
	return number_of_elements;
}

void
PageResource«BreRule»::setNumberOfElements(int  number_of_elements)
{
	this->number_of_elements = number_of_elements;
}

int
PageResource«BreRule»::getSize()
{
	return size;
}

void
PageResource«BreRule»::setSize(int  size)
{
	this->size = size;
}

std::list<Order>
PageResource«BreRule»::getSort()
{
	return sort;
}

void
PageResource«BreRule»::setSort(std::list <Order> sort)
{
	this->sort = sort;
}

long long
PageResource«BreRule»::getTotalElements()
{
	return total_elements;
}

void
PageResource«BreRule»::setTotalElements(long long  total_elements)
{
	this->total_elements = total_elements;
}

int
PageResource«BreRule»::getTotalPages()
{
	return total_pages;
}

void
PageResource«BreRule»::setTotalPages(int  total_pages)
{
