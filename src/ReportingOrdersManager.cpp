...
#include "ReportingOrdersManager.h"
#include "NetClient.h"
#include "Helpers.h"
#include "Error.h"
...
ReportingOrdersManager::ReportingOrdersManager()
{
...
static gboolean __ReportingOrdersManagerresponseHandler(gpointer data)
{
...
static gpointer __ReportingOrdersManagerthreadFunc(gpointer data)
{
	RequestInfo *request = static_cast<RequestInfo*>(data);
	...
	g_idle_add(__ReportingOrdersManagerresponseHandler, static_cast<gpointer>(request));
	...
	, void* userData, bool isAsync)
{
	...
	if(!isAsync){
		NetClient::easycurl(ReportingOrdersManager::getBasePath(), url, myhttpmethod, queryParams,
		...
		requestInfo = new(nothrow) RequestInfo (ReportingOrdersManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg, userData, reinterpret_cast<void(*)()>(handler), getInvoiceReportsProcessor);;
		if(requestInfo == NULL)
			return false;

		thread = g_thread_new(NULL, __ReportingOrdersManagerthreadFunc, static_cast<gpointer>(requestInfo));
		return true;
	}
}
...
bool ReportingOrdersManager::getInvoiceReportsAsync(char * accessToken,
	std::string currencyCode, std::string granularity, std::string filterPaymentStatus, std::string filterFulfillmentStatus, long long startDate, long long endDate, int size, int page, 
	void(* handler)(PageResource«AggregateInvoiceReportResource», Error, void* )
	, void* userData)
{
...
bool ReportingOrdersManager::getInvoiceReportsSync(char * accessToken,
	std::string currencyCode, std::string granularity, std::string filterPaymentStatus, std::string filterFulfillmentStatus, long long startDate, long long endDate, int size, int page, 
	void(* handler)(PageResource«AggregateInvoiceReportResource», Error, void* )
	, void* userData)
{
