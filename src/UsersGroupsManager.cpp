...
static bool updateGroupsProcessor(MemoryStruct_s p_chunk, long code, char* errormsg, void* userData,
	void(* voidHandler)())
{
...
static bool updateGroupsHelper(char * accessToken,
	...
	, void* userData, bool isAsync)
{
	...
	if(!isAsync){
		NetClient::easycurl(UsersGroupsManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg);
		bool retval = updateGroupsProcessor(*p_chunk, code, errormsg, userData,reinterpret_cast<void(*)()>(handler));
		...
		requestInfo = new(nothrow) RequestInfo (UsersGroupsManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg, userData, reinterpret_cast<void(*)()>(handler), updateGroupsProcessor);;
		if(requestInfo == NULL)
			return false;
...
bool UsersGroupsManager::updateGroupsAsync(char * accessToken,
	std::string filterTemplate, std::string filterMemberCount, std::string filterName, std::string filterUniqueName, std::string filterParent, std::string filterStatus, int size, int page, std::string order, 
	void(* handler)(PageResource«GroupResource», Error, void* )
	, void* userData)
{
	return updateGroupsHelper(accessToken,
	filterTemplate, filterMemberCount, filterName, filterUniqueName, filterParent, filterStatus, size, page, order, 
	handler, userData, true);
}

bool UsersGroupsManager::updateGroupsSync(char * accessToken,
	std::string filterTemplate, std::string filterMemberCount, std::string filterName, std::string filterUniqueName, std::string filterParent, std::string filterStatus, int size, int page, std::string order, 
	void(* handler)(PageResource«GroupResource», Error, void* )
	, void* userData)
{
