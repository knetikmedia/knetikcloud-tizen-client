...
static bool searchQuestionTagsProcessor(MemoryStruct_s p_chunk, long code, char* errormsg, void* userData,
	void(* voidHandler)())
{
	void(* handler)(Collection«string», Error, void* )
	= reinterpret_cast<void(*)(Collection«string», Error, void* )> (voidHandler);
	...
	if (code >= 200 && code < 300) {
		...
		if (isprimitive("Collection«string»")) {
			pJson = json_from_string(data, NULL);
			jsonToValue(&out, pJson, "Collection«string»", "Collection«string»");
			json_node_free(pJson);

			if ("Collection«string»" == "std::string") {
				string* val = (std::string*)(&out);
				if (val->empty() && p_chunk.size>4) {
					*val = string(p_chunk.memory, p_chunk.size);
				}
