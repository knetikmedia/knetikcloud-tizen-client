...
void
PageResource«ArticleResource»::__init()
{
	//
...
void
PageResource«ArticleResource»::__cleanup()
{
	//if(content != NULL) {
...
void
PageResource«ArticleResource»::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<ArticleResource> new_list;
			ArticleResource inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("ArticleResource")) {
					jsonToValue(&inst, temp_json, "ArticleResource", "");
				} else {
...
PageResource«ArticleResource»::PageResource«ArticleResource»(char* json)
{
	this->fromJson(json);
}

char*
PageResource«ArticleResource»::toJson()
{
	JsonObject *pJsonObject = json_object_new();
	JsonNode *node;
	if (isprimitive("ArticleResource")) {
		list<ArticleResource> new_list = static_cast<list <ArticleResource> > (getContent());
		node = converttoJson(&new_list, "ArticleResource", "array");
	} else {
		...
		for (list<ArticleResource>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			ArticleResource obj = *it;
...
std::list<ArticleResource>
PageResource«ArticleResource»::getContent()
{
	return content;
}

void
PageResource«ArticleResource»::setContent(std::list <ArticleResource> content)
{
	this->content = content;
}

bool
PageResource«ArticleResource»::getFirst()
{
	return first;
}

void
PageResource«ArticleResource»::setFirst(bool  first)
{
	this->first = first;
}

bool
PageResource«ArticleResource»::getLast()
{
	return last;
}

void
PageResource«ArticleResource»::setLast(bool  last)
{
	this->last = last;
}

int
PageResource«ArticleResource»::getNumber()
{
	return number;
}

void
PageResource«ArticleResource»::setNumber(int  number)
{
	this->number = number;
}

int
PageResource«ArticleResource»::getNumberOfElements()
{
	return number_of_elements;
}

void
PageResource«ArticleResource»::setNumberOfElements(int  number_of_elements)
{
	this->number_of_elements = number_of_elements;
}

int
PageResource«ArticleResource»::getSize()
{
	return size;
}

void
PageResource«ArticleResource»::setSize(int  size)
{
	this->size = size;
}

std::list<Order>
PageResource«ArticleResource»::getSort()
{
	return sort;
}

void
PageResource«ArticleResource»::setSort(std::list <Order> sort)
{
	this->sort = sort;
}

long long
PageResource«ArticleResource»::getTotalElements()
{
	return total_elements;
}

void
PageResource«ArticleResource»::setTotalElements(long long  total_elements)
{
	this->total_elements = total_elements;
}

int
PageResource«ArticleResource»::getTotalPages()
{
	return total_pages;
}

void
PageResource«ArticleResource»::setTotalPages(int  total_pages)
{
