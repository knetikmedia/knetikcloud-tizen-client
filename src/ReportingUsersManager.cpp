...
#include "ReportingUsersManager.h"
#include "NetClient.h"
#include "Helpers.h"
#include "Error.h"
...
ReportingUsersManager::ReportingUsersManager()
{
...
static gboolean __ReportingUsersManagerresponseHandler(gpointer data)
{
...
static gpointer __ReportingUsersManagerthreadFunc(gpointer data)
{
	RequestInfo *request = static_cast<RequestInfo*>(data);
	...
	g_idle_add(__ReportingUsersManagerresponseHandler, static_cast<gpointer>(request));
	...
	, void* userData, bool isAsync)
{
	...
	if(!isAsync){
		NetClient::easycurl(ReportingUsersManager::getBasePath(), url, myhttpmethod, queryParams,
		...
		requestInfo = new(nothrow) RequestInfo (ReportingUsersManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg, userData, reinterpret_cast<void(*)()>(handler), getUserRegistrationsProcessor);;
		if(requestInfo == NULL)
			return false;

		thread = g_thread_new(NULL, __ReportingUsersManagerthreadFunc, static_cast<gpointer>(requestInfo));
		return true;
	}
}
...
bool ReportingUsersManager::getUserRegistrationsAsync(char * accessToken,
	std::string granularity, long long startDate, long long endDate, int size, int page, 
	void(* handler)(PageResource«AggregateCountResource», Error, void* )
	, void* userData)
{
...
bool ReportingUsersManager::getUserRegistrationsSync(char * accessToken,
	std::string granularity, long long startDate, long long endDate, int size, int page, 
	void(* handler)(PageResource«AggregateCountResource», Error, void* )
	, void* userData)
{
