...
void
PageResource«ArtistResource»::__init()
{
	//
...
void
PageResource«ArtistResource»::__cleanup()
{
	//if(content != NULL) {
...
void
PageResource«ArtistResource»::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<ArtistResource> new_list;
			ArtistResource inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("ArtistResource")) {
					jsonToValue(&inst, temp_json, "ArtistResource", "");
				} else {
...
PageResource«ArtistResource»::PageResource«ArtistResource»(char* json)
{
	this->fromJson(json);
}

char*
PageResource«ArtistResource»::toJson()
{
	JsonObject *pJsonObject = json_object_new();
	JsonNode *node;
	if (isprimitive("ArtistResource")) {
		list<ArtistResource> new_list = static_cast<list <ArtistResource> > (getContent());
		node = converttoJson(&new_list, "ArtistResource", "array");
	} else {
		...
		for (list<ArtistResource>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			ArtistResource obj = *it;
...
std::list<ArtistResource>
PageResource«ArtistResource»::getContent()
{
	return content;
}

void
PageResource«ArtistResource»::setContent(std::list <ArtistResource> content)
{
	this->content = content;
}

bool
PageResource«ArtistResource»::getFirst()
{
	return first;
}

void
PageResource«ArtistResource»::setFirst(bool  first)
{
	this->first = first;
}

bool
PageResource«ArtistResource»::getLast()
{
	return last;
}

void
PageResource«ArtistResource»::setLast(bool  last)
{
	this->last = last;
}

int
PageResource«ArtistResource»::getNumber()
{
	return number;
}

void
PageResource«ArtistResource»::setNumber(int  number)
{
	this->number = number;
}

int
PageResource«ArtistResource»::getNumberOfElements()
{
	return number_of_elements;
}

void
PageResource«ArtistResource»::setNumberOfElements(int  number_of_elements)
{
	this->number_of_elements = number_of_elements;
}

int
PageResource«ArtistResource»::getSize()
{
	return size;
}

void
PageResource«ArtistResource»::setSize(int  size)
{
	this->size = size;
}

std::list<Order>
PageResource«ArtistResource»::getSort()
{
	return sort;
}

void
PageResource«ArtistResource»::setSort(std::list <Order> sort)
{
	this->sort = sort;
}

long long
PageResource«ArtistResource»::getTotalElements()
{
	return total_elements;
}

void
PageResource«ArtistResource»::setTotalElements(long long  total_elements)
{
	this->total_elements = total_elements;
}

int
PageResource«ArtistResource»::getTotalPages()
{
	return total_pages;
}

void
PageResource«ArtistResource»::setTotalPages(int  total_pages)
{
