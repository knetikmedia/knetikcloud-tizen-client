...
#include "MessagingTopicsManager.h"
#include "NetClient.h"
#include "Helpers.h"
#include "Error.h"
...
MessagingTopicsManager::MessagingTopicsManager()
{
...
static gboolean __MessagingTopicsManagerresponseHandler(gpointer data)
{
...
static gpointer __MessagingTopicsManagerthreadFunc(gpointer data)
{
	RequestInfo *request = static_cast<RequestInfo*>(data);
	...
	g_idle_add(__MessagingTopicsManagerresponseHandler, static_cast<gpointer>(request));
	...
	void(* handler)(Error, void* ) , void* userData, bool isAsync)
{
	...
	if(!isAsync){
		NetClient::easycurl(MessagingTopicsManager::getBasePath(), url, myhttpmethod, queryParams,
		...
		requestInfo = new(nothrow) RequestInfo (MessagingTopicsManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg, userData, reinterpret_cast<void(*)()>(handler), disableTopicSubscriberProcessor);;
		if(requestInfo == NULL)
			return false;

		thread = g_thread_new(NULL, __MessagingTopicsManagerthreadFunc, static_cast<gpointer>(requestInfo));
		return true;
	}
}
...
bool MessagingTopicsManager::disableTopicSubscriberAsync(char * accessToken,
	std::string id, std::string userId, ValueWrapper«boolean» disabled, 
...
bool MessagingTopicsManager::disableTopicSubscriberSync(char * accessToken,
	std::string id, std::string userId, ValueWrapper«boolean» disabled, 
...
bool MessagingTopicsManager::getTopicSubscriberAsync(char * accessToken,
	std::string id, std::string userId, 
	void(* handler)(TopicSubscriberResource, Error, void* )
	, void* userData)
{
...
bool MessagingTopicsManager::getTopicSubscriberSync(char * accessToken,
	std::string id, std::string userId, 
	void(* handler)(TopicSubscriberResource, Error, void* )
	, void* userData)
{
...
static bool getTopicSubscribersProcessor(MemoryStruct_s p_chunk, long code, char* errormsg, void* userData,
	void(* voidHandler)())
{
	void(* handler)(PageResource«TopicSubscriberResource», Error, void* )
	= reinterpret_cast<void(*)(PageResource«TopicSubscriberResource», Error, void* )> (voidHandler);
	
	JsonNode* pJson;
	char * data = p_chunk.memory;

	
	PageResource«TopicSubscriberResource» out;
	...
	if (code >= 200 && code < 300) {
		...
		if (isprimitive("PageResource«TopicSubscriberResource»")) {
			pJson = json_from_string(data, NULL);
			jsonToValue(&out, pJson, "PageResource«TopicSubscriberResource»", "PageResource«TopicSubscriberResource»");
			json_node_free(pJson);

			if ("PageResource«TopicSubscriberResource»" == "std::string") {
				string* val = (std::string*)(&out);
				if (val->empty() && p_chunk.size>4) {
					*val = string(p_chunk.memory, p_chunk.size);
				}
...
static bool getTopicSubscribersHelper(char * accessToken,
	std::string id, 
	void(* handler)(PageResource«TopicSubscriberResource», Error, void* )
	, void* userData, bool isAsync)
{
	...
	string url("/messaging/topics/{id}/subscribers");
	int pos;
	...
	if(!isAsync){
		NetClient::easycurl(MessagingTopicsManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg);
		bool retval = getTopicSubscribersProcessor(*p_chunk, code, errormsg, userData,reinterpret_cast<void(*)()>(handler));
		...
		requestInfo = new(nothrow) RequestInfo (MessagingTopicsManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg, userData, reinterpret_cast<void(*)()>(handler), getTopicSubscribersProcessor);;
		if(requestInfo == NULL)
			return false;
...
bool MessagingTopicsManager::getTopicSubscribersAsync(char * accessToken,
	std::string id, 
	void(* handler)(PageResource«TopicSubscriberResource», Error, void* )
	, void* userData)
{
	return getTopicSubscribersHelper(accessToken,
	id, 
	handler, userData, true);
}

bool MessagingTopicsManager::getTopicSubscribersSync(char * accessToken,
	std::string id, 
	void(* handler)(PageResource«TopicSubscriberResource», Error, void* )
	, void* userData)
{
...
bool MessagingTopicsManager::getUserTopicsAsync(char * accessToken,
	std::string id, 
	void(* handler)(PageResource«TopicResource», Error, void* )
	, void* userData)
{
...
bool MessagingTopicsManager::getUserTopicsSync(char * accessToken,
	std::string id, 
	void(* handler)(PageResource«TopicResource», Error, void* )
	, void* userData)
{
